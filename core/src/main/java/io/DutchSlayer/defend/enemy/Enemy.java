...
public class Enemy {
    ...
    private void updateAnimation(float delta) {
        ...
        if ((type == EnemyType.BASIC || type == EnemyType.SHIELD || type == EnemyType.SHOOTER || type == EnemyType.BOMBER) &&
            ...
            if (animationTimer >= animSpeed) {
                ...
                System.out.println("🎬 " + type + " animation frame changed to: " + currentFrame +
                    " (speed: " + animSpeed + "s)");
            }
        }
    ...
    private Texture getCurrentTexture() {
        ...
        if (type == EnemyType.BASIC) {
            System.out.println("🎯 Getting texture for BASIC enemy, frame: " + currentFrame);

            if (ImageLoader.enemyBasicFrames == null) {
                System.out.println("❌ enemyBasicFrames is NULL! Animation frames not loaded!");
            ...
            if (currentFrame >= 0 && currentFrame < ImageLoader.enemyBasicFrames.length) {
                ...
                if (frameTexture != null) {
                    System.out.println("✅ Using BASIC animation frame: " + currentFrame);
                    return frameTexture;
                } else {
                    System.out.println("❌ BASIC Animation frame " + currentFrame + " is NULL!");
                }
            }

            System.out.println("⚠️ Using BASIC fallback static texture");
            return ImageLoader.enemyBasicTex != null ? ImageLoader.enemyBasicTex : ImageLoader.dutchtex;
        }

        // ===== TAMBAHAN: SHIELD ENEMY ANIMATION =====
        else if (type == EnemyType.SHIELD) {
            System.out.println("🛡️ Getting texture for SHIELD enemy, frame: " + currentFrame);

            if (ImageLoader.enemyShieldFrames == null) {
                System.out.println("❌ enemyShieldFrames is NULL! Animation frames not loaded!");
            ...
            if (currentFrame >= 0 && currentFrame < ImageLoader.enemyShieldFrames.length) {
                ...
                if (frameTexture != null) {
                    System.out.println("✅ Using SHIELD animation frame: " + currentFrame);
                    return frameTexture;
                } else {
                    System.out.println("❌ SHIELD Animation frame " + currentFrame + " is NULL!");
                }
            }

            System.out.println("⚠️ Using SHIELD fallback static texture");
            return ImageLoader.enemyShieldTex != null ? ImageLoader.enemyShieldTex : ImageLoader.dutchtex;
        }

        // ===== TAMBAHAN: SHOOTER ENEMY ANIMATION =====
        else if (type == EnemyType.SHOOTER) {
            System.out.println("🏹 Getting texture for SHOOTER enemy, frame: " + currentFrame);

            if (ImageLoader.enemyShooterFrames == null) {
                System.out.println("❌ enemyShooterFrames is NULL! Animation frames not loaded!");
            ...
            if (currentFrame >= 0 && currentFrame < ImageLoader.enemyShooterFrames.length) {
                ...
                if (frameTexture != null) {
                    System.out.println("✅ Using SHOOTER animation frame: " + currentFrame);
                    return frameTexture;
                } else {
                    System.out.println("❌ SHOOTER Animation frame " + currentFrame + " is NULL!");
                }
            }

            System.out.println("⚠️ Using SHOOTER fallback static texture");
            return ImageLoader.enemyShooterTex != null ? ImageLoader.enemyShooterTex : ImageLoader.dutchtex;
        }

        // ===== TAMBAHAN: BOMBER ENEMY ANIMATION =====
        else if (type == EnemyType.BOMBER) {
            System.out.println("💣 Getting texture for BOMBER enemy, frame: " + currentFrame);

            if (ImageLoader.enemyBomberFrames == null) {
                System.out.println("❌ enemyBomberFrames is NULL! Animation frames not loaded!");
            ...
            if (currentFrame >= 0 && currentFrame < ImageLoader.enemyBomberFrames.length) {
                ...
                if (frameTexture != null) {
                    System.out.println("✅ Using BOMBER animation frame: " + currentFrame);
                    return frameTexture;
                } else {
                    System.out.println("❌ BOMBER Animation frame " + currentFrame + " is NULL!");
                }
            }

            System.out.println("⚠️ Using BOMBER fallback static texture");
            return ImageLoader.enemyBomberTex != null ? ImageLoader.enemyBomberTex : ImageLoader.dutchtex;
        }
